...
app = FastAPI(
    ...
    description='基于 LangGraph 的多智能体 HR 服务接口',    # API 描述
)
...
async def chat_endpoint(request:ChatRequest):
    ...
    try:
        for event in hr_agent_app.stream(state,config,stream_mode='values'):
            last_messages = event['messages'][-1]

            if isinstance(last_messages,HumanMessage):
                continue

            if last_messages.type == 'ai' and not last_messages.tool_calls:
                final_answer = last_messages.content
...
if __name__ == '__main__':
    ...
    # uvicorn.run()  # 启动服务器
    # api:app        # api 是当前文件名，app是Fast API 实例
    # host='0.0.0.0'   # 监听所有网络接口，允许外部访问
    # port=80000       # 监听 8000 端口
