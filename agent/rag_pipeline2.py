...
class QueryExpansion(BaseModel):
    expanded_question:list[str] = Field(description='从不同维度扩写3个相关检索词或短语')
...
def expand_and_hyde(original_query:str) -> list[str]:
    """利用LLM生成多维度扩写问题与HydDE假设性长文"""
    prompt = ChatPromptTemplate.from_template(
        "你是一个专业的企业HR专家。为了提高知识库的检索命中率，请协助处理用户的原始提问。\n"
        "任务一（多维度扩写）：站在不同的视角（如政策名词、审批流程、系统操作）扩写3个相关的查询短语。\n"
        "任务二（HyDE假设）：用官方、严谨的HR规章制度口吻，伪造一段回答该问题的文本。不管事实是否正确，重点是模仿‘员工手册’的专业行文风格。\n\n"
        "用户原始提问：{query}\n\n"
        "{format_instructions}"
    )
    ...
    try:
        ...
        print(f'\n原始问题：{original_query}')
        print(f'   -> 衍生问题：{result['expanded_questions']}')
        print(f'   -> HyDE伪文：{result['hypothetical_document']}')

        # 汇总：原问题 + 3个衍生问题 + 1个假设性文档
