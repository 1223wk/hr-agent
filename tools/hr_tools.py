...
def get_employee_profile(uid:str) -> str:
    ...
    if not res:
        return f'ERROR:未找到UID为{uid}的员工信息。'
    emp = res[0]
...
def check_leave_balance(uid:str) -> str:
    """
    根据员工 UID 查询其剩余假期余额（包括年假和病假）
    当员工明确询问‘我还有几天假’或者‘我的假期余额’时使用
    """
    sql = """
...
def generate_employment_certificate(uid:str,cert_type:str) -> str:
    ...
    参数 cert_type 必须是一下两个值之一：
    - 'employment':仅开具在职证明（全员可用）
    - 'income':开具包含薪资的在职及收入证明（有职级权限限制，仅 P5 及以上可用）
    """
    emp_res = query_db(conn=db_conn,sql="select name,rank,base_salary from employees where uid = ?",params=(uid,))
    if not emp_res:
